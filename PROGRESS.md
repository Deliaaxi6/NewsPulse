...
### 2026-08-20 — 全市场覆盖修复（北交所代码后缀 + 板块动态涨停上限）
- ✅ 北交所代码后缀（提交 c29744d）：_code() 原实现非 6 开头全标 XSHE，北交所（4/8/920）错误；现 6→XSHG（含科创 688）、0/3→XSHE（含创业板）、其余→BJ；测试 +5 用例，服务器 7 段实测全对
- ✅ 板块动态涨停上限（提交 5e25f0b）：快照过滤原 9.9% 上限排除创业板/科创板 10~20% 涨幅与北交所 5~30% 涨幅（盲区，如 688836 宇树科技上市次日起 15% 涨幅无法入选）
  - _pct_max：主板 9.9 / 300·301·688·689 19.9 / 4·8·920 29.9；_filter_market_df 按代码逐行动态上限（不追已涨停）
  - 实测（服务器真实快照 3000 只）：北交所 89 / 科创板 441 / 创业板 997——北交所经 m:0 t:81 已入快照，涨停池仍仅沪深（36 只实测无北交所）
  - 测试 +12 用例（8 段映射 + 4 过滤场景）；本地 25 套件 + 服务器 ALL PASSED
- 📌 设计边界（非缺陷）：北交所/科创 30%/20% 涨停股不追（涨停价剔除）；3000 只涨幅降序截断仅极端普涨日可能漏尾部低涨幅股

### 2026-08-20（续）— 快照全量分页（3000→6000）与 data:null 尾页容错
- ✅ 全量分页（提交 c2ffb45）：EM_CLIST_PAGES / SINA_SPOT_PAGES 30→60，空页自动提前停（sina break / em_alt 提前返回），极端普涨日不再漏尾部低涨幅股
- 🔥 实测发现东财接口行为：pn 超出股票总量后 clist 返回 `data:null`（非空 diff）——60 页场景必触发，原 `["data"]["diff"]` 索引抛 TypeError 误判整源失败；另 83.push2 节点现被封（与 82/push2 同命运），仅 push2delay 可用
- ✅ 修复（提交 770a633）：data/diff 容错（`(data or {}).get("diff") or []`），尾部 data:null 提前返回（首页为空仍抛，走 try_chain 切源）；测试 +3 用例（data:null 首页抛/尾页提前返回）
- ✅ 服务器实测：em_alt 全量 5900 只 / 16.4s（首源 em 官方仍被封自动降级）、sina 5547 只 / 19.6s；本地 25 套件 ALL PASSED

### 2026-08-20（续2）— 北交所涨停收录 + sim_account 早盘真实行情口径
- ✅ 北交所 30% 涨停收录（提交 6c337df）：东财涨停池仅沪深，北交所涨停股原无进池路径；_pct_max 北交所 29.9→30.1（涨停价也收录，快照唯一路径）；沪深维持不追（涨停池已覆盖，避免重复）
- ✅ sim_account 新浪实时行情（提交 6a0ab5f）：日K降级链路早盘最新 bar 为昨收 → pct=0 → 涨跌停保护失效；改为 hq.sinajs.cn 实时批量（现价/昨收真实 pct、含开高低、一次请求全池；前缀 6→sh、0/3→sz、4/8/920→bj；失败仍降级腾讯分时）
- ✅ 测试 +8 用例（实时解析/北交所前缀/缺字段/网络失败）；服务器实测真实行情：茅台 -1.23%、五粮液 +0.21%、北交所 920001 正常返回
- ✅ 本地 25 套件 ALL PASSED；git push origin main:master（01ade9d..6a0ab5f）
...
- ✅ 修复（提交 7634687）：_market_pool 降级链 [em → em_alt → sina]，retries=2：
  - em_alt：83.push2/push2delay 轮换翻页（fid=f3 涨幅降序 30 页），每页失败自动换子域（应对节点随机性）；实测 3000 只 11.5s，过滤后 204 只
  - sina：按涨幅排序分页快照（东财全挂时兜底；早盘 9:05 前数据为 0 值，盘后有效）
  - 涨停池仍为最终兜底；本地 21 套件 + 服务器实测 PASS
- ⚠️ 已知：新浪源早盘 0 值（盘前无成交数据）；东财节点封锁为出口 IP 层问题，若换代理/节点可能恢复

### 2026-08-16 — 数据积累里程碑（接口深度实测：新闻不可回溯，只能逐日积累）
- 事实：东财快讯接口仅返回最近 200 条（≈2 天）、个股新闻仅 10 条——超过 2 天的历史新闻物理上拿不到；历史行情 K 线 akshare 全量可拉（已充足，无需补）
- 里程碑（按交易日历，节假日顺延）：
  - 08-17（周一）9:05 首日自动运行，开始逐日积累
  - 08-22（周六）首次周复盘输出（数据 ≥5 日）
  - 约 09-10（20 个交易日）ML 风控因子自动启用（daily_sentiment ≥20 行 + portfolio 序列）
  - 约 09 月上旬（2-3 周数据）回测参数验证可执行
- 备选加速：Tushare Pro 积分新闻接口（付费，未引入）

### 2026-08-16 — tech_factor 历史快照修复（asof 截断）
...
### 2026-08-20 �� ������ȫ��·���鸲�ǣ�������� + ʵ���޸���
- ��ƽ��ۣ�ѡ��/�����Ѹ���ȫ�� A �ɰ�飻��ǰʵ��ȷ�ϱ����� 920001 ��K ���ã�akshare bj ǰ׺ 883 �У����� sh/sz ����ǰ׺�� 4/8/920 ���� sz �� ����/ָ��/����/���߽���/�ز���·ȫ��ʧ�ܣ�KeyError�����������ɽ��۲�ص���Զ���ź�
- �޸���config.sina_prefix()��6��sh ���ƴ���0/3��sz ����ҵ�������bj��ͳһ 6 �����ã�strategies.fetch_kline / cyq.fetch_daily / indicators.fetch_daily / decision.fetch_spot_sina / backtest.load_hist_quotes / sim_account.latest_quote_sina
- ʵ�⣨����������fetch_kline 920001=300�С�indicators=120�С�cyq=180�С�ʵʱ�� 11.8��-0.08%��?����Ʊ���� stock_news_em(920001)=10 �� ?
- ����ä������Ѷ��ʱ��adata qq����֧�ֱ�������ʵ�� EMPTY����������+����ͬʱʧ��ʱ����������������fail-open��
- �ύ��47321bc / 85dc533�����ԣ�test_strategies +8���� sina_prefix ���� + fetch_kline bj �հ����ԣ���ȫ���ع� ALL PASSED

### 2026-09-02 �� ӯ���ձ��������� + ��Ѷ���ױ�����
- ���� src/pnl_report.py���� portfolio ���գ��ھ�=�ֽ�+��(shares>0 ����ֵ)���ۼ�ӯ�� vs INIT_CASH/����/�ֲָ�ӯ��ϸ�������� fail-open ��������
- run_all �ӵ� 7 ����ӯ������daily_report ֮�󣩣�˫ͨ�� send_text + send_to_channel��test_pnl_report 7 ���� + test_run_all ������Ը���
- �޸���Ѷ���ף�latest_quote_qq ��Ϊ qt.gtimg.cn ������adata qq �Ա����� EMPTY ʵ��ȷ�ϣ����������� bj ǰ׺+pct
- ��������ʵ������֤��ӯ���ձ���� ���ʲ� 100,336 / �ۼ�ӯ�� +336(+0.34%) / �� 08-28 +166 / �ֲ� 4 ֻ��ϸ
- �ύ ff9ec9e / d1f89b8���� push + ���𣩣�ȫ���ع� ALL PASSED��27 �׼���

### 2026-09-02 �� ӯ��/�ֲֶϵ��޸���fail-open + ������ʲ���
- �޸� sim_account ����ȫ�վ�Ĭ return����Ϊ���ֲֳɱ��۹�ֵд�У������С����󱨿ղ֡����ɽ�����08-21 �ϵ���������
- ��ǰ���ս�����latest_quote_sina/qq �ּ�=0 ʱ�����գ�pct=0����9:05 ��ǰ���鲻�ٶ����
- �޸� positions_rows total_value �ھ���ÿ��д������ʲ����ֽ�+��ȫ���ֲ���ֵ�����ǵ�����ֵ���۶϶� rows[-1] ���ٵ͹������ϣ����� 3 ���ܸ������۶���֣�
- ������������֤������ȫ�� �� д 2600��@28262 ��ֵ ���ʲ� 28518.6 ��������ʵ���� positions_rows ȫ��=100,335.6
- tests: trading 37/37��������ǰ����/������ʲ�����ȫ���ع� ALL PASSED
- �ύ a0729e8���� push + ����

### 2026-09-02 �� load_portfolio �� date_str ��ȡ����
- �޸� load_portfolio ��ǽ�� today �ж����¸���/�زⰴ��ʷ������ʱ����"����"�� ���ؿղ�+10�����á��ֲ�ȫ��
- ��Ϊ load_portfolio(asof=date_str)����ȡ date<=asof ������Ϊ�˻���㣻asof=None ����ǽ�ӣ����ݾɵ��ã���main �� date_str
- backtest ���� portfolio.csv ����ϣ�ȫ���ع� ALL PASSED��trading 38/38 ���� asof ������
- ��������ʵ������֤��asof=09-02 ���� 4 ʵ�ʳֲ�/�ֽ�256.6��asof=08-20 �������տ���/�ֽ�80065.55
- �ύ b4cb8bc���� push + ����
