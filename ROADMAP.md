...
9. 告警：复用 Telegram 通知脚本 send_telegram.py
...
| 通知 | 现有 send_telegram.py | 复用已验证脚本 |
...
│  ⑤ daily_report.py 生成HTML报告+Telegram 推送      │
│       → reports/report_{date}.html            │
└──────────────────────────────────────────────┘
交互模型：Telegram 报告(主) + CSV查看(次) + 手动运行命令
```
...
**熔断机制**：3倍杠杆下市值跌超 33%（归零）→ 触发"爆仓熔断"：清仓 + Telegram 告警 + 停止自动交易。待满足冷却条件自动恢复：至少 5 个交易日冷静期 + 连续 2 日情绪分转正 + 恢复后仅 1 倍杠杆起步。

**交易规则**：信号当日 9:05 实时价成交（近似开盘价）；T+1；佣金万2.5+卖出印花税0.05%（最低5元）；单票仓位上限30%；总仓位上限90%；涨停不卖/跌停不买。
...
| 7 | daily_report.py HTML 报告 + Telegram 推送 | 报告可打开，消息可达 |
