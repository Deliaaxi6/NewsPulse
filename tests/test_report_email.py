...
def main() -> int:
    ...
    old = daily_report.TG_SCRIPT
    daily_report.TG_SCRIPT = r"C:\不存在的脚本\send_telegram.py"
    try:
        ok = daily_report.send_report_tg("2026-08-13",
                                         Path(r"C:\不存在的报告\report_2026-08-13.html"))
        check("脚本缺失返回False不抛异常", ok is False)
    finally:
        daily_report.TG_SCRIPT = old

    # 2. 空日期/正常参数 → 脚本存在时正常执行（真机验证，此处仅确认接口可调用）
    check("接口签名可调用", callable(daily_report.send_report_tg))

    # 3. 模拟报告路径不存在时仍不抛异常（脚本存在场景，用 python -c 桩替代）
    import tempfile
    with tempfile.TemporaryDirectory() as td:
        stub = Path(td) / "send_tg_stub.py"
        stub.write_text(
            "import sys, pathlib\n"
            "p = pathlib.Path(sys.argv[sys.argv.index('--attachment')+1])\n"
            "sys.exit(1 if not p.exists() else 0)\n",
            encoding="utf-8")
        daily_report.TG_SCRIPT = str(stub)
        try:
            ok = daily_report.send_report_tg("2026-08-13",
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        daily_report.TG_SCRIPT = str(stub)
        try:
            ok = daily_report.send_report_tg("2026-08-13", rep)
            check("桩脚本成功返回True", ok is True)
        finally:
            daily_report.TG_SCRIPT = old

    print(f"push: {4 - fails}/4 passed")
