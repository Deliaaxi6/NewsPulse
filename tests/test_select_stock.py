...
def main() -> int:
    ...
    # --- 板块动态涨幅上限（不追已涨停）：主板9.9 / 创业科创19.9 / 北交所29.9 ---
    check("动态上限 主板", select_stock._pct_max("600519") == 9.9)
    check("动态上限 创业板300", select_stock._pct_max("300750") == 19.9)
    ...
    board_snap = pd.DataFrame([
        ...
        {"代码": "920001", "名称": "北交", "涨跌幅": 25.0, "量比": 2.5, "成交额": 5e8},
        {"代码": "600001", "名称": "主板", "涨跌幅": 15.0, "量比": 2.5, "成交额": 5e8},
        {"代码": "300751", "名称": "创业涨停", "涨跌幅": 20.0, "量比": 2.5, "成交额": 5e8},
        {"代码": "920002", "名称": "北交涨停", "涨跌幅": 30.0, "量比": 2.5, "成交额": 5e8},
    ])
